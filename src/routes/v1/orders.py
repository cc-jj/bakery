...
@router.patch("/items/", response_model=schemas.Order)
def update_order_item(order_item: schemas.OrderItemEdit, db: Session = Depends(get_db)):
    return crud.update_order_item(db, order_item)
...
def update_order(order: schemas.OrderEdit, db: Session = Depends(get_db)):
